...
async function check() {
  try {
    const res = await fetch(`https://graph.facebook.com/v19.0/${config.FACEBOOK_PAGE_ID}/scheduled_posts?access_token=${config.FACEBOOK_ACCESS_TOKEN}&fields=id,message,scheduled_publish_time,is_published,attachments,full_picture,source,permalink_url&limit=5`);
